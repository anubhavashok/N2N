...
if datasetName is 'mnist':
    ...
    model = torch.load('./parent_models/mnist_new2.net')
    baseline_acc = 0.994
    ...
    model = torch.load('./parent_models/cifar10_new.net')
    baseline_acc = 0.92
...
controller = nn.LSTM(num_input, num_hidden, num_layers, bidirectional=True)
opti = optim.Adam(controller.parameters(), lr=0.003)
Wt_softmax = nn.Linear(num_hidden*2, num_output)
softmax = nn.Softmax()
...
def build_child_model(featureLayers, classifierLayers, actions):
    ...
    inp = featureLayers[0]._layer.cuda()(inp)
    ...
    if numInputsToFC < 10 or numInputsToFC > LINEAR_THRESHOLD:
        return None
...
def rolloutActions(featureLayers, classifierLayers):
    ...
    output, (hn, cn) = controller(input, (hn, cn))
    probs = softmax(Wt_softmax(output.squeeze(1)))
...
def rollout(model_, i):
    ...
    featureLayers = list(filter(lambda x: x.type in [1, 2], featureLayers))
