...
from controllers.LSTM import LSTM
...
if datasetName is 'mnist':
    ...
    model = torch.load('./parent_models/resnet18_cifar100.net')
    ...
    baseline_acc = 0.9525
    datasetInputTensor = torch.Tensor(1, 3, 32, 32)
    model = torch.load('./parent_models/svhn_resnet18.net')
    modelSavePath = './protos_svhn/'
    controllerSavePath = './controllers_svhn/lstm_resnet18.net'
    controllerLoadPath = './controllers_svhn/lstm_resnet18.net'
...
def fixLayers(m):
    #m.classifier._modules['0'].fixed = True
...
def rolloutActions(layers):
    ...
    actions = controller(input, (hn, cn))
    return actions
...
def rollout_batch(model, N, e):
    ...
    for i in range(N):
        ...
        if hashcode in previousModels and constrained == False:
            ...
            idxs.append(i)
    accs = trainNormalParallel(studentModels, dataset, epochs=5) if datasetName is 'caltech256' else trainTeacherStudentParallel(model, studentModels, dataset, epochs=5)
    for acc in accs:
        print('Val accuracy: %f' % acc)
